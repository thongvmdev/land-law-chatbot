...
export class CloudflareEmbeddings extends Embeddings {
  ...
  private async embedTexts(texts: string[]): Promise<number[][]> {
    ...
    try {
      ...
      // Cloudflare returns embeddings in result.data format
      ...
      if (error instanceof Error) {
        throw new Error(
          `Failed to get embeddings from Cloudflare AI: ${error.message}`,
        )
      }
      throw new Error(
        'Failed to get embeddings from Cloudflare AI: Unknown error',
      )
    }
  }
}
